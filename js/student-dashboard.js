(() => {
    ...
    const money=(amount,currency="NGN")=>Number(amount||0).toLocaleString("en-NG",{style:"currency",currency,minimumFractionDigits:2});

    async function loadCarryForwardBalance(){
        const box=$("#carry-forward-balance");
        const note=$("#carry-forward-note");
        if(!box||!profile?.current_level)return;
        const current=String(profile.current_level||"").trim();
        const {data:history}=await hrSupabase.from("student_level_history").select("from_level,to_level,academic_year,created_at").eq("student_id",currentUser.id).order("created_at",{ascending:true});
        const previousLevels=new Set((history||[]).map(r=>String(r.from_level||"").trim()).filter(Boolean).filter(l=>l!==current));
        if(!previousLevels.size){
            box.innerHTML=`<div class="metric"><span>Previous school fees</span><b>${money(0)}</b></div><div class="metric"><span>Previous examination fees</span><b>${money(0)}</b></div><div class="metric"><span>Total carried forward</span><b>${money(0)}</b></div>`;
            if(note)note.textContent="No previous-level outstanding balance has been recorded.";
            return;
        }
        const [{data:fees,error:feeError},{data:payments,error:payError}]=await Promise.all([
            hrSupabase.from("fee_catalog").select("id,name,category,amount,currency,school_id,level,exam_type,active"),
            hrSupabase.from("payments").select("id").eq("payer_user_id",currentUser.id).eq("status","successful")
        ]);
        if(feeError||payError){if(note)note.textContent="Unable to calculate carried-forward balances right now.";return;}
        const ids=(payments||[]).map(p=>p.id);
        let allocations=[];
        if(ids.length){const {data}=await hrSupabase.from("payment_allocations").select("fee_id,allocation_type,level,semester,exam_type,amount,payment_id").eq("payer_user_id",currentUser.id).in("payment_id",ids);allocations=data||[];}
        let school=0, exam=0;
        for(const fee of (fees||[])){
            if(fee.school_id && profile.school_id && fee.school_id !== profile.school_id) continue;
            const level=String(fee.level||"").trim();
            if(!level||!previousLevels.has(level))continue;
            const rows=allocations.filter(a=>a.fee_id===fee.id&&String(a.level||"").trim()===level);
            if(String(fee.category||"").toLowerCase()==="school_fees"){
                const annual=Number(fee.amount||0); const first=Math.round(annual/2*100)/100; const second=Math.round((annual-first)*100)/100;
                const firstPaid=rows.filter(a=>a.allocation_type==="school_fee"&&a.semester==="First Semester").reduce((x,a)=>x+Number(a.amount||0),0);
                const secondPaid=rows.filter(a=>a.allocation_type==="school_fee"&&a.semester==="Second Semester").reduce((x,a)=>x+Number(a.amount||0),0);
                school+=Math.max(0,Math.round((annual-firstPaid-secondPaid)*100)/100);
            } else if(String(fee.category||"").toLowerCase()==="examination"){
                const type=String(fee.exam_type||fee.name||"").trim();
                const paid=rows.filter(a=>a.allocation_type==="examination"&&String(a.exam_type||"").trim()===type).reduce((x,a)=>x+Number(a.amount||0),0);
                exam+=Math.max(0,Math.round((Number(fee.amount||0)-paid)*100)/100);
            }
        }
        const total=school+exam;
        box.innerHTML=`<div class="metric"><span>Previous school fees</span><b>${money(school)}</b></div><div class="metric"><span>Previous examination fees</span><b>${money(exam)}</b></div><div class="metric"><span>Total carried forward</span><b>${money(total)}</b></div>`;
        if(note)note.textContent=total>0.01?"This balance came from your previous academic level(s) and remains payable after promotion. Open Payments & Fees to settle it.":"No previous-level outstanding balance has been recorded.";
    }
    ...
    async function init(){if(!window.hrSupabase)return;if(!await loadStudent())return;await Promise.all([loadResults(),loadHistory(),loadApplication(),loadNotices(),loadCarryForwardBalance()]);}
