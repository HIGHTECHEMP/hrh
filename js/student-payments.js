(() => {
      ...
      if (isSchoolFee(fee)) {
        ...
        cards.push(`
          <article class="fee-card fee-card-school">
            ...
            <button type="button" class="btn primary carry-pay-fee-btn" data-fee-id="${fee.id}" data-fee-type="school" data-carry-forward="true" data-source-level="${escapeHtml(fee.level)}">Pay outstanding fees</button>
          </article>`);
  ...
  async function handlePayment(button) {
    ...
    const fee =
      feeCatalog.find((item) => item.id === feeId) ||
      carryForwardFees.find((item) => item.id === feeId);
