...
function previousLevels(student) {
  const current = levelNumber(student.current_level);
  if (!current) return [];
  return cashierFees
    .map(fee => String(fee.level || "").trim())
    .filter(Boolean)
    .filter(level => levelNumber(level) > 0 && levelNumber(level) < current)
    .filter((level, index, arr) => arr.indexOf(level) === index)
    .sort((a, b) => levelNumber(a) - levelNumber(b));
}

function carryForwardEntries(student) {
  const levels = previousLevels(student);
  const entries = [];
  for (const fee of cashierFees) {
    const level = String(fee.level || "").trim();
    if (!level || !levels.includes(level) || fee.school_id && fee.school_id !== student.school_id) continue;
    const rows = allocationsFor(student, fee).filter(a => String(a.level || "").trim() === level);
    if (String(fee.category || "").toLowerCase() === "school_fees") {
      const annual = Number(fee.amount || 0);
      const firstDue = semesterAmount(annual, "First Semester");
      const secondDue = semesterAmount(annual, "Second Semester");
      ...
      const outstanding = Math.max(0, Math.round((annual-firstPaid-secondPaid)*100)/100);
      if (outstanding > 0.01) entries.push({type:"school",fee,level,firstPaid,secondPaid,firstDue,secondDue,outstanding});
      ...
      const outstanding = Math.max(0, Math.round((Number(fee.amount||0)-paid)*100)/100);
      if (outstanding > 0.01) entries.push({type:"examination",fee,level,paid,outstanding,examType:type});
    }
  }
  return entries;
}

function carryForwardTotal(student) {
  return carryForwardEntries(student).reduce((sum, item) => sum + Number(item.outstanding || 0), 0);
}
...
function renderStudent(
  ...
  const carryEntries = carryForwardEntries(student);
  const carryTotal = carryEntries.reduce((sum, item) => sum + Number(item.outstanding || 0), 0);
  ...
  return `
    <article class="cashier-student-card">
      ...
      ${carryEntries.length ? `
        <div class="cashier-section-label">CARRIED-FORWARD OUTSTANDING</div>
        <div class="cashier-exam-list">
          ${carryEntries.map(item => {
            if (item.type === "school") {
              return `<div class="cashier-exam-row"><div><span>School fees from ${esc(item.level)}</span><small>${money(item.outstanding)} outstanding</small></div><div class="cashier-progress"><span style="width:${Math.min(100, ((item.outstanding <= 0 ? 1 : 0)) * 100)}%"></span></div><b class="unpaid">Carry forward</b></div>`;
            }
            return `<div class="cashier-exam-row"><div><span>${esc(item.examType)} from ${esc(item.level)}</span><small>${money(item.outstanding)} outstanding</small></div><div></div><b class="unpaid">Carry forward</b></div>`;
          }).join("")}
        </div>
        <div class="cashier-ledger-total"><span>Total carried forward</span><b>${money(carryTotal)}</b><small>Previous-level balance still payable after promotion</small></div>
      ` : `
        <div class="cashier-section-label">CARRIED-FORWARD OUTSTANDING</div>
        <div class="cashier-unconfigured">No previous-level outstanding balance.</div>
      `}
